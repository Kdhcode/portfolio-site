...
modal.addEventListener('click', function (e) { // e: 이벤트 발생 시 이벤트 객체가 전달됨
  ...
  console.log(e.currentTarget); // 이벤트가 바인딩된 요소(여기선 modal)
  
  if (e.target === e.currentTarget) { // 이벤트 리스너가 붙은 요소 그자체를 클릭한 경우만 실행
    modal.style.display = 'none';
  }
});
...
// 페이지 스크롤 이벤트 감지를 추가!
// 브라우저는 문서 젠체의 스크롤을 window 기준으로 처리(doccument에 붙이면 일부 브라우저에서는 동작 안 함)
// window : 브라우저 창 객체
